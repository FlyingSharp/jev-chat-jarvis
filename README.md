...
- 或选择「屏幕 OCR」：不开无障碍也可以使用，首次会由 Android 弹出一次“录制屏幕”授权；之后由前台服务定时截图并在本机用 ML Kit 识别
- 悬浮窗 / 显示在其他应用上层（展示分析）
...
- 判断接口内置 OpenRouter、TypeSafe 直连、DeepSeek 官方（Chat Completions JSON 适配）预设；回复接口内置 OpenRouter、DeepSeek 官方、通义兼容预设；视觉接口内置 OpenRouter、DeepSeek 官方（`deepseek-flash`）、通义兼容预设，每张卡均可一键连通测试。
...
- **OCR 依赖系统放行截屏**：可通过无障碍截屏，或在主页授权「屏幕 OCR」使用 Android MediaProjection；受保护窗口（`FLAG_SECURE`）截不到。
